(function(){
  ...
  function profile(){
    try{
      ...
      if(!u) return null;
      const keys=['nutritrack_v2_'+u.uid,'nutritrack_v2'];
      let d={};
      for(const k of keys){
        try{const raw=localStorage.getItem(k);if(raw){d=JSON.parse(raw)||{};if(d.profile||d.uid||d.email)break;}}catch(_){}
      }
      const merged={...(d.profile||{}),...d,uid:u.uid,email:(u.email||d.profile?.email||d.email||'').toLowerCase()};
      return merged;
    }catch(e){return null;}
  }
  function isPro(p){
    if(!p)return false;
    if(window.NS_PREMIUM_ACTIVE===true)return true;
    if(window.nsIsPremium){try{if(window.nsIsPremium(window.firebaseAuth?.currentUser,p))return true;}catch(_) {}}
    const email=String(p.email||'').toLowerCase().trim();
    return p.uid===UID_PRO||email===EMAIL_PRO||p.isPremium===true||p.plan==='pro'||p.subscriptionStatus==='active'||p.subscription?.status==='active';
  }
  function calc(p){
  ...
  function meal(p,c){
    ...
    const portions={
      ...
      definicion:[['Claras de huevo',180],['Avena',35],['Fresas',150],['Pechuga de pollo',170],['Papa cocida',180],['Brócoli',150],['Atún en agua',120],['Tortilla de maíz',2],['Aguacate',40]],
  ...
  function inject(){
    ...
    if(!body)return;
    const isDash=Array.from(document.querySelectorAll('button')).some(b=>b.textContent.trim()==='Inicio');
